...
class Climate:
    ...
    def plot_all_data(self):
        """ plot mean yearly temperatures for all cities for all years """
        temperatures = {}
        for city in self.rawdata:
            for year in self.rawdata[city]:
                try:
                    temperatures[year] += np.mean(self.get_yearly_temp(city, year))
                except KeyError:
                    temperatures[year] = np.mean(self.get_yearly_temp(city, year))
...
def generate_models(x, y, degs):
    ...
    """
    return [np.array(np.polyfit(x, y, d)) for d in degs]
...
def evaluate_models_on_training(x, y, models, title):
    ...
    """
    for model in models:
        est_y_vals = np.polyval(model, x)
        plt.plot(x, y, 'bo', label='Measured points')
        plt.plot(x, est_y_vals, 'r', label='Linear fit, r**2 = ' + str(round(r_squared(y, est_y_vals), 4)))
        plt.title(title)
        plt.xlabel('Year')
...
if __name__ == "__main__":
    ...
    for year in INTERVAL_1:
        y.append(raw_data.get_daily_temp(city, month, day, year))
    plot_title = f"Daily temp on {month}.{day} in {city.title()}"
    models = generate_models(x, y, [1])
    evaluate_models_on_training(x, y, models, plot_title)
    ...
    plot_title = f"Yearly temp in {city.title()}"
    models = generate_models(x, y, [1])
    evaluate_models_on_training(x, y, models, plot_title)

    # raw_data.plot_all_data()
